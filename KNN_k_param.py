...
def knn(train_data,train_labels,test_data,k):
	...
	for i in test_data:
		...
		for j in range(len(train_data)):
			...
			# Finds the indes of the minimum numeric value in the minDist list.
			toReplace = minDist.index(min(minDist))
			if currDist > minDist[toReplace]:
				minDist[toReplace] = currDist
				minDistIndex[toReplace] = j
		labels = [train_labels[l] for l in minDistIndex]
		# Finds the largest value in a set of the labels, using the number of times they appear as the value
		# and then returning the label itself.
		chosen_label = max(set(labels), key = labels.count)
...
if __name__ == "__main__":
	...
	predictions = knn(train_data,train_labels,test_data,1)
